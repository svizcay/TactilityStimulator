...
{
    [CreateAssetMenu(fileName = "newFingerCircularVE", menuName = "Tactility/Virtual Electrode/Finger Circular")]
    public class FingerCircularVE : VirtualElectrode8
    {
        // dictionary's key is the connectorId
        // then, to get the channel for pad=3, we access the array at position pad-1
        Dictionary<int, int[]> mapping = new Dictionary<int, int[] >()
        {
            { 3, new int[]{2,4,6,3,1,5,7,8} },
            { 4, new int[]{9,10,11,12,13,14,15,16} },
        };
...
#if UNITY_EDITOR
    [CustomEditor(typeof(FingerCircularVE))]
    public class FingerCircularVEEditor : Editor
    {
